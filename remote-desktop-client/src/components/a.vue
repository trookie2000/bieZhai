  ...
  ws.onmessage = async (e: MessageEvent) => {
    ...
    switch (msg.msg_type) {
      ...
      case MessageType.CLOSE_REMOTE_DESKTOP:
        const videoId = msg.receiver; // 假设消息中包含了需要关闭的视频 ID
        closeVideoStreamById(videoId); // 调用函数来关闭特定 ID 的视频流
        break;
    }
  };
...
function closeVideoStreamById(videoId) {
  const element = document.getElementById(videoId);
  if (element && element instanceof HTMLVideoElement) {
    const videoElement = element;
    if (videoElement.srcObject instanceof MediaStream) {  // 确保srcObject是MediaStream
    ...
    console.error('Provided element is not a video element');
  }
}
...
<template>
  ...
  <div v-if="data.isConnecting" class="connecting-message sidebarr"
    style="position: fixed; top: 0; left: 0; right: 0; bottom: 0;">正在被远控...</div>
